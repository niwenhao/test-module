...
open class ApiDaoImpl(
    ...
    override fun getUserApiList(userId: Long): List<ProviderApi> {
        ...
        val apis = user.providerApis.map { api ->
            ProviderApi(api.id, null, api.apiPath, api.apiName, api.conditionJson, api.responseJson, mutableListOf())
        }
    ...
    override fun create(userId: Long, api: ProviderApi): ProviderApi? {
        return em.find(ProviderUser::class.java, userId)?.let { user ->
            val napi = ProviderApi(null, user, api.apiPath, api.apiName, api.conditionJson, api.responseJson, mutableListOf())
            em.persist(napi)
            em.detach(napi)
            napi.user = null
    ...
    override fun remove(apiId: Long): ProviderApi? {
        return em.find(ProviderApi::class.java, apiId)?.let { api ->
            val r = ProviderApi(api.id, null, api.apiPath, api.apiName, api.conditionJson, api.responseJson, mutableListOf())
            em.remove(api)
            r.user = null
